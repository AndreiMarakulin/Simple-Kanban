...
exports.seed = async function (knex) {
  ...
  await knex("user").insert([
    { login: "admin", password: await bcrypt.hash("admin", 5), name: "admin" },
    { login: "user1", password: await bcrypt.hash("user1", 5), name: "user1" },
    { login: "user2", password: await bcrypt.hash("user2", 5), name: "user2" },
  ]);
  ...
  await knex("board_list").insert([
    ...
    { board_id: 1, list_id: 3 },
    { board_id: 2, list_id: 4 },
    { board_id: 2, list_id: 5 },
    { board_id: 2, list_id: 6 },
  ]);
  ...
  await knex("list_card").insert(
    Array.from({ length: 30 }).map((_, idx) => {
      return {
        card_id: idx + 1, list_id: getRndInteger(1, 7),
      };
    })
  );
};
